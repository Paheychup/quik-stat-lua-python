...
function OnInit()
	...
	--filename="IPADDRESS_"..tostring(getInfoParam("IPADDRESS")).."_UID_"..tostring(getInfoParam("USERID")).."_"..tostring(os.date("%Y%m%d")).."_"..tostring(timeformat(getInfoParam("LOCALTIME"))).."_CETS.txt"
	filename=tostring(getInfoParam("IPCOMMENT")).."_IPADDRESS_"..tostring(getInfoParam("IPADDRESS")).."_UID_"..tostring(getInfoParam("USERID")).."_"..tostring(os.date("%Y%m%d")).."_"..tostring(timeformat(getInfoParam("LOCALTIME"))).."_CETS.txt"
	trans_send_order=0
	trans_kill_order=0
...
function last()
	...
	--message("func last value="..tostring(last))
	...
	if isConnected()==1 then
		if TRADINGSTATUS==1 then -- проверка, что Статус сессии - торгуется
			if LAST > CHECK_PRICE then --проверка, что текущая цена выше минимально возможной
				...
				if (local_time>=100100 and local_time<=135800) or (local_time>=140900 and local_time<=183500) then
...
function OnTransReply(trans_reply)
	...
	if trans_send_order==1 and trans_kill_order==0 then
		...
		elseif status==3 then
			...
			message_send_order=("send_order;"..trans_id_ord..";"..tostring(local_time)..";"..order_num..";"..tostring(send_order_stop_time)..";"..tostring(send_order_start_time)..";"..tostring(send_order_stop_time-send_order_start_time)..";CETS;"..tostring(os.date("%Y%m%d")).."\n")
	...
	elseif trans_kill_order==1 and trans_send_order==1  then
		...
		elseif status==3 then
			...
			message_kill_order=("kill_order;"..trans_id_ord..";"..tostring(local_time)..";"..order_num..";"..tostring(kill_order_stop_time)..";"..tostring(kill_order_start_time)..";"..tostring(kill_order_stop_time-kill_order_start_time)..";CETS;"..tostring(os.date("%Y%m%d")).."\n")
			trans_send_order=0 
			trans_kill_order=0
